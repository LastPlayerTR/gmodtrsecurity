...
router.get("/", async (req: (express.Request<ParamsDictionary, any, any, any, Record<string, any>> & { session: { discord: DiscordStrategy.Profile, steam: any }  }), res) => {
     ...
     if (member.roles.cache.has(roles.manual)) return await next_app.render(req, res, "/error", { text: "Hesabın manuel olarak doğrulanacak, Discord üzerinden bir yetkiliyle iletişime geç." })
     if (member.roles.cache.has(roles.registered)) return await  next_app.render(req, res, "/error", { text: "Hesabın zaten doğrulanmış." })

     const matchedConnection = (req.session.discord.connections || []).find(x => x.type === "steam" && x.id === req.session.steam.id)
     
     if (!matchedConnection) return await next_app.render(req, res, "/error", { text: "Discord hesabında bu Steam hesabın yok, Discord'da hesap ayarlarından bağlantılar kısmından Steam hesabını bağlamayı unutma!" })
     ...
     if ((Date.now() - member.user.createdTimestamp) < 2.592e+9 || (Date.now() - req.session.steam.creationdate) < 2.592e+9) {
          ...
          return await next_app.render(req, res, "/error", { text: "Hesabın manuel olarak doğrulanacak, Discord üzerinden bir yetkiliyle iletişime geç." })
     }

     const playTime: number | null = await new Promise<number>(async (resolve, reject) => {
          let games: any[] = await fetch(`http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${apiKey}&steamid=${req.session.steam.id}&format=json`).then(x => x.json()).then(x => x.response.games)
          if (!games) reject()
          let game = games.find(x => x.appid === 4000)
          ...
          if (game.playtime_forever < minPlayMins) {
               ...
               await next_app.render(req, res, "/error", { text: "Gmod süren yeterli olmadığı için uymadığı için (+"+  (minPlayMins / 60) + " saat) hesabın manuel olarak doğrulanacak,Discord üzerinden bir yetkiliyle iletişime geç." })
               resolve(null)
          }
          ...
          await next_app.render(req, res, "/error", { text: "Hesabından oyun bilgileri alınamadığı için, hesabın manuel olarak doğrulanacak , Discord üzerinden bir yetkiliyle iletişime geç." })
          return null
     })
